...
function elementTextWithBreaks($: cheerio.CheerioAPI, el: Parameters<cheerio.CheerioAPI>[0] | cheerio.CheerioAPI): string {
  const $el = $(el as any);
  const html = $el.html();
        ...
        for (const node of items) {
          ...
          if (type === 'Product' || (Array.isArray(type) && type.includes('Product'))) {
            ...
            result.brand =
              result.brand ||
              cleanText(
                node.brand?.name || node.brand?.value || node.brand
              );
            result.description = result.description || htmlToPlainText(node.description);
            if (node.image) {
...
function extractDescription($: cheerio.CheerioAPI): string {
  const meta = findMeta($, ['og:description', 'twitter:description', 'description']);
...
function extractBrand(name: string, $: cheerio.CheerioAPI): string {
  ...
  if (siteName) {
    ...
    const retailerNames = /Blade HQ|Knife Center|GP Knives|DLT Trading|KnifeWorks|KnivesPlus|Knife Art|KnifeJoy|NC Blade|Southern Edges|House of Blades|New Graham Knives|True North Knives|Steel Addiction|USA Made Blade/i;
    if (!retailerNames.test(siteName)) return siteName;
  }

  const commonBrands = [
    'Chris Reeve', 'Spyderco', 'Benchmade', 'Microtech', 'Hinderer',
    'Grimsmo', 'Pro-Tech', 'ZT', 'Zero Tolerance', 'Bark River',
    'Boker', 'Böker', 'Buck', 'Case', 'Cold Steel', 'CRKT',
    'Demko', 'Emerson', 'Fox', 'GiantMouse', 'Kershaw', 'Liong Mah',
    'Medford', 'MKM', 'Nemesis', 'Olamic', 'QuietCarry', 'Reate',
    'Rike', 'Shirogorov', 'Smith & Wesson', 'Spartan Blades',
    'Toor', 'Tops', 'Viper', 'WE Knife', 'WE', 'CIVIVI', 'Kizer',
    'Kizer Cutlery', 'Bestech', 'Civivi', 'QSP', 'Kansept', 'Petrified Fish',
    'Sencut', 'Twosun', 'CJRB', ' Artisan ', 'Artisan Cutlery', 'Real Steel',
    'James Brand', 'Opinel', 'Victorinox', 'Leatherman', 'SOG',
    'Brous Blades', 'Chaves', 'CRK', 'Hogue', 'Midgards-Messer',
    'Miguron', 'North Arms', 'Pena', 'Rassenti', 'Rogers',
    'Sergey Rogovets', 'Skiff', 'SBD', 'Strider', 'Tactile Turn',
    'Terrain 365', 'Trevor Burger', 'Vero Engineering', 'Vosteed',
  ];
...
const COUNTRIES = [
  'USA', 'United States', 'China', 'Taiwan', 'Japan', 'Italy', 'Germany',
  'France', 'Spain', 'Sweden', 'Finland', 'Norway', 'Denmark', 'Austria',
  'Czech Republic', 'Russia', 'South Africa', 'Canada', 'Mexico', 'Brazil',
  'Argentina', 'Australia', 'New Zealand', 'UK', 'United Kingdom', 'England',
];

function normalizeCountry(value: string): string {
  const clean = cleanText(value);
  for (const country of COUNTRIES) {
    if (new RegExp(`\\b${country.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'i').test(clean)) {
...
function parseSpecLines(text: string, onPair: (label: string, value: string) => void) {
  if (!text) return;
...
function extractSpecBlock($: cheerio.CheerioAPI): string {
  const headingSelectors = 'h1, h2, h3, h4, h5, h6, strong, b, p, div, span, dt, th';
  const $heading = $(headingSelectors)
    .filter((_, el) => {
      const t = $(el).text().trim();
      return /^(specifications?|specs?|details?|technical\s+(?:data|specs|details)|product\s+(?:specs|details|specifications))$/i.test(t);
    })
    .first();
...
function extractSpecs($: cheerio.CheerioAPI): ScrapedProduct['specs'] {
  ...
  const setSpec = (key: keyof ScrapedProduct['specs'], value: string) => {
    ...
    } else if (key === 'bladeLength' || key === 'overallLength' || key === 'bladeThickness' || key === 'handleLength') {
  ...
  const labelSelectors = 'th, dt, .spec-label, [class*="label"], .attribute-label, .pdp-specs-label, [class*="specification-data-title"], [class*="specification-title"]';
  const valueSelectors = 'td, dd, .spec-value, [class*="value"], .attribute-value, .pdp-specs-value, [class*="specification-data-value"], [class*="specification-value"]';
  ...
  const pageText = normalizeMultilineText(elementTextWithBreaks($, 'body' as any));
  const allText = `${ldDescription}\n${metaDescription}\n${pageText}`;

  const runFallback = (key: keyof ScrapedProduct['specs'], pattern: RegExp, transform: (v: string) => string) => {
    if (specs[key]) return;
  ...
  runFallback('bladeLength', /\bblade\s*(?:length|size|len)\s*[:\uFF1A]\s*([\d.,/]+\s*(?:"|''|in|inches|mm|cm)?)/i, normalizeLength);
  runFallback('overallLength', /\boverall\s*(?:length|size|len)\s*[:\uFF1A]\s*([\d.,/]+\s*(?:"|''|in|inches|mm|cm)?)/i, normalizeLength);
  runFallback('handleLength', /\bhandle\s*(?:length|size|len)\s*[:\uFF1A]\s*([\d.,/]+\s*(?:"|''|in|inches|mm|cm)?)/i, normalizeLength);
  runFallback('bladeThickness', /\bblade\s*(?:thickness|thick)\s*[:\uFF1A]\s*([\d.,/]+\s*(?:"|''|in|inches|mm|cm)?)/i, normalizeLength);
  runFallback('weight', /\bweight\s*[:\uFF1A]\s*([\d.,]+\s*(?:oz|g|grams|lbs|lb|kg)\b)/i, normalizeWeight);
  runFallback('bladeMaterial', /\bblade\s*(?:material|steel)\s*[:\uFF1A]\s*(.{1,40}?)(?=\n|$)/i, cleanValue);
  runFallback('bladeCoating', /\bblade\s*(?:coating|finish|finishing)\s*[:\uFF1A]\s*(.{1,40}?)(?=\n|$)/i, cleanValue);
  runFallback('lockingMechanism', /\b(?:locking\s*mechanism|lock\s*type|lock\s*mechanism)\s*[:\uFF1A]\s*(.{1,40}?)(?=\n|$)/i, cleanValue);
  runFallback('designer', /\b(?:designer|designed\s*by)\s*[:\uFF1A]\s*(.{1,40}?)(?=\n|$)/i, cleanValue);
  runFallback('modelNumber', /\b(?:model\s*(?:number|#|no\.?)|sku)\s*[:\uFF1A]\s*(.{1,40}?)(?=\n|$)/i, cleanValue);
  runFallback('hardness', /\b(?:hardness|hrc|rockwell)\s*[:\uFF1A]\s*([\d.-]+\s*(?:hrc|rockwell)?)/i, cleanValue);

  // Country needs special normalization.
  if (!specs.country) {
    const countryMatch = allText.match(/\b(?:country\s*of\s*origin|made\s*in)\s*[:\uFF1A]\s*([A-Za-z\s]+?)(?=\n|\.|\s{2,}|$)/i);
    if (countryMatch) {
...
const HANDLE_MATERIALS = [
  'Natural Canvas Micarta', 'Linen Micarta', 'Canvas Micarta', 'Micarta',
  'Carbon Fiber', 'G-10', 'G10', 'FRN', 'PEI', 'Ultem',
  'Titanium', 'Aluminum', 'Aluminium', 'Brass', 'Copper', 'Bronze',
  'Stabilized Wood', 'Wood', 'Bone', 'Stag', 'Mother of Pearl', 'Abalone',
  'Rubber', 'Kraton', 'Grivory', 'Nylon', 'Polymer',
];
...
const BLADE_STYLES = [
  'Reverse Tanto', 'Drop Point', 'Clip Point', 'Tanto',
  'Spear Point', 'Sheepsfoot', 'Wharncliffe', 'Hawkbill',
  'Dagger', 'Spanto', 'Trailing Point', 'Needle Point',
  'Pen Blade', 'Warncliffe', 'Harpoons', 'Hawksbill',
...
export function extractShopifyProduct(json: unknown): Partial<ScrapedProduct> | null {
  const product = (json as { product?: Record<string, unknown> })?.product;
  if (!product || typeof product !== 'object') return null;

  const title = typeof product.title === 'string' ? cleanText(product.title) : '';
  const vendor = typeof product.vendor === 'string' ? cleanText(product.vendor) : '';
  ...
  if (Array.isArray(rawImages)) {
    for (const img of rawImages) {
      if (typeof img === 'object' && img && typeof (img as { src?: string }).src === 'string') {
...
export function scrapeProduct(
  ...
  const handleMaterial =
    cleanValue(extractLabeledValue(productDescriptionText, ['handle material'])) ||
