...
function findMacApp() {
  ...
  if (!fs.existsSync(desktopRoot)) {
    throw new Error('dist/desktop does not exist. Build a macOS desktop release first.');
  }
...
function main() {
  ...
  if (!isSignedRelease) {
    ...
    console.log('Expected behavior: Gatekeeper will block direct first launch from a downloaded DMG.');
    console.log('Tester install flow: copy the app out of the DMG, remove quarantine, then open it.');
    return;
  }
